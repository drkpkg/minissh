...
// historyLimit bounds how many scrolled-off primary-screen rows are
// retained for scrollback. At ~24 bytes/Glyph and a generous 200-column
// terminal, 1000 rows is ~4.8MB per session — trivial for the handful of
// concurrent embedded sessions minissh runs.
...
type State struct {
	...
	lines         []line
	altLines      []line
	history       []line // scrolled-off primary-screen rows, oldest first
...
// HistoryLen returns the number of scrolled-off primary-screen rows
// retained for scrollback (oldest first). Always 0 on the alt screen,
// since alt-screen apps (less, vim, top, ...) manage their own display and
// nothing is captured while ModeAltScreen is set.
func (t *State) HistoryLen() int {
	return len(t.history)
}

// HistoryLine returns a copy of scrollback row i (0 = oldest), with the
// same color-override resolution Cell applies. Panics if i is out of
// [0, HistoryLen()) — callers are expected to check HistoryLen first.
func (t *State) HistoryLine(i int) []Glyph {
	row := t.history[i]
	out := make([]Glyph, len(row))
	copy(out, row)
	for x, g := range out {
		if fg, ok := t.colorOverride[g.FG]; ok {
			g.FG = fg
		}
		if bg, ok := t.colorOverride[g.BG]; ok {
			g.BG = bg
		}
		out[x] = g
	}
...
func (t *State) scrollUp(orig, n int) {
	n = clamp(n, 0, t.bottom-orig+1)
	// Only rows leaving the true top of the whole screen (not some
	// DECSTBM-restricted scroll region lower down, and not IL/DL commands
	// scrolling mid-screen) are gone for good and worth remembering; alt-
	// screen apps (less, vim, top, ...) manage their own display, so
	// nothing is captured while ModeAltScreen is set.
	if orig == 0 && t.top == 0 && t.mode&ModeAltScreen == 0 {
		t.pushHistory(n)
	}
...
// pushHistory appends the n rows currently at the top of the screen
// (t.lines[0:n]) to the scrollback buffer, deep-copying each row's cells —
// scrollUp/scrollDown reuse the same backing arrays by swapping line
// headers rather than allocating fresh ones, so a shallow copy would end
// up silently mutated by later scrolling.
func (t *State) pushHistory(n int) {
	if n <= 0 {
		return
	}
	rows := make([]line, n)
	for i := 0; i < n; i++ {
		src := t.lines[i]
		row := make(line, len(src))
		copy(row, src)
		rows[i] = row
	}
	t.history = append(t.history, rows...)
	if over := len(t.history) - historyLimit; over > 0 {
		// Reallocate rather than reslice so the dropped rows' backing
		// arrays are actually freed instead of held live by the slice's
		// original capacity.
		t.history = append([]line(nil), t.history[over:]...)
	}
}
